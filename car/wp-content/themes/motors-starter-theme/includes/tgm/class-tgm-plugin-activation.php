				...
				class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function after( $title = '' ) {
						if ( $this->error || ! $this->result ) {
							if ( $this->error ) {
								$format = isset( $this->upgrader->strings['skin_update_failed_error'] ) ? $this->upgrader->strings['skin_update_failed_error'] : '%1$s failed with error: %2$s';
								if ( substr_count($format, '%') === 2 ) {
									$after_error_message = sprintf( $format, $title, '<strong>' . $this->error . '</strong>' );
								} else {
									$after_error_message = sprintf( $format, $title );
								}
							} else {
								$format = isset( $this->upgrader->strings['skin_update_failed'] ) ? $this->upgrader->strings['skin_update_failed'] : '%s failed';
								if ( substr_count( $format, '%' ) === 1 ) {
									$after_error_message = sprintf( $format, $title );
								} else {
									$after_error_message = $format;
								}
							}
							wp_admin_notice(
								$after_error_message,
